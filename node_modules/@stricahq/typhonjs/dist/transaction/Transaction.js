...
class Transaction {
    constructor({ protocolParams }) {
        ...
        this.plutusDataList = [];
        this._isPlutusTransaction = false;
        this.mints = [];
    ...
    addInput(input) {
        if (input.address.paymentCredential.type === types_1.HashType.ADDRESS) {
            this.requiredWitnesses.set(input.address.paymentCredential.hash, input.address.paymentCredential.bipPath);
        }
    ...
    addCertificate(certificate) {
        if (certificate.certType === types_1.CertificateType.STAKE_DELEGATION) {
            if (certificate.stakeCredential.type === types_1.HashType.ADDRESS) {
                this.requiredWitnesses.set(certificate.stakeCredential.hash, certificate.stakeCredential.bipPath);
            }
        }
    ...
    addOutput(output) {
        ...
        if (uOutput.plutusData) {
            const encodedPlutusData = cbors.Encoder.encode((0, encoder_1.encodePlutusData)(uOutput.plutusData));
            uOutput.plutusDataHash = (0, crypto_1.hash32)(encodedPlutusData).toString("hex");
        }
    ...
    addWithdrawal(withdrawal) {
        if (withdrawal.rewardAccount.stakeCredential.type === types_1.HashType.ADDRESS) {
            this.requiredWitnesses.set(withdrawal.rewardAccount.stakeCredential.hash, withdrawal.rewardAccount.stakeCredential.bipPath);
        }
        this.withdrawals.push(withdrawal);
    }
    transactionBody({ extraOutputs, scriptDataHash, }) {
        const encodedBody = new Map();
        encodedBody.set(types_1.TransactionBodyItemType.INPUTS, (0, encoder_1.encodeInputs)(this.inputs));
        let trxOutputs = this.outputs;
        if (extraOutputs && extraOutputs.length > 0) {
            trxOutputs = trxOutputs.concat(extraOutputs);
        }
        encodedBody.set(types_1.TransactionBodyItemType.OUTPUTS, (0, encoder_1.encodeOutputs)(trxOutputs));
        encodedBody.set(types_1.TransactionBodyItemType.FEE, this.fee);
        if (this.ttl !== undefined) {
            encodedBody.set(types_1.TransactionBodyItemType.TTL, this.ttl);
        }
        if (this.validityIntervalStart !== undefined) {
            encodedBody.set(types_1.TransactionBodyItemType.VALIDITY_INTERVAL_START, this.validityIntervalStart);
        }
        if (this.certificates.length > 0) {
            encodedBody.set(types_1.TransactionBodyItemType.CERTIFICATES, (0, encoder_1.encodeCertificates)(this.certificates));
        }
        if (this.withdrawals.length > 0) {
            encodedBody.set(types_1.TransactionBodyItemType.WITHDRAWALS, (0, encoder_1.encodeWithdrawals)(this.withdrawals));
        }
        if (this.auxiliaryData) {
            ...
            encodedBody.set(types_1.TransactionBodyItemType.AUXILIARY_DATA_HASH, auxiliaryDataHash);
        }
        if (!lodash_1.default.isEmpty(this.mints)) {
            encodedBody.set(types_1.TransactionBodyItemType.MINT, (0, encoder_1.encodeMint)(this.mints));
        }
        if (scriptDataHash) {
            encodedBody.set(types_1.TransactionBodyItemType.SCRIPT_DATA_HASH, scriptDataHash);
        }
        if (!lodash_1.default.isEmpty(this.collaterals)) {
            encodedBody.set(types_1.TransactionBodyItemType.COLLATERAL_INPUTS, (0, encoder_1.encodeCollaterals)(this.collaterals));
        }
        const requiredSigners = Array.from(this.requiredSigners.keys());
        if (!lodash_1.default.isEmpty(requiredSigners)) {
            encodedBody.set(types_1.TransactionBodyItemType.REQUIRED_SIGNERS, requiredSigners.map((key) => buffer_1.Buffer.from(key, "hex")));
        }
        return encodedBody;
    }
    transactionFee(size) {
    ...
    calculateTxSize(extraOutputs) {
        ...
        const scriptDataHash = (0, helpers_1.generateScriptDataHash)(encodedWitnesses, this._protocolParams.languageView);
    ...
    calculateMinUtxoAmount(tokens, hasPlutusDataHash) {
        return (0, utils_1.calculateMinUtxoAmount)(tokens, this._protocolParams.lovelacePerUtxoWord, hasPlutusDataHash);
    }
    addWitness(witness) {
