...
const encodeOutput = (output) => {
    ...
    const encodedOutput = [output.address.getBytes(), amount];
...
const encodeStakeRegistrationCertificate = (certificate) => {
    const stakeKeyHash = buffer_1.Buffer.from(certificate.stakeCredential.hash, "hex");
...
const encodeWitnesses = (vKeyWitness, inputs, plutusDataList, plutusScriptMap, nativeScripts, mints) => {
    ...
    if (encodedPlutusScriptsV1.length) {
        encodedWitnesses.set(types_1.WitnessType.PLUTUS_SCRIPT, encodedPlutusScriptsV1);
    }
...
const encodeLanguageViews = (languageView, encodedPlutusScripts) => {
